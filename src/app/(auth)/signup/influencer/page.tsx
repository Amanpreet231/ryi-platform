  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error: signUpError } = await supabase.auth.signUp({
        ...
        options: { data: { full_name: formData.fullName, phone: formData.phone, user_type: 'influencer' } },
      });
    ...
    <div className="relative min-h-screen bg-black flex items-center justify-center px-4 py-12 overflow-hidden">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="mt-1.5 text-sm text-zinc-500">Start finding brand deals today — free forever</p>
        </div>
        ...
        <div className="bg-zinc-900/80 border border-zinc-800 backdrop-blur-sm rounded-2xl p-7">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input name="fullName" type="text" placeholder="Rahul Sharma" value={formData.fullName} onChange={handleChange} required
                className="w-full bg-zinc-900/50 border border-zinc-800 text-white placeholder-zinc-600 rounded-xl px-4 py-3 text-sm focus:border-zinc-600 focus:outline-none transition-colors" />
            </div>
            <div>
              <label className="block text-sm font-medium text-zinc-300 mb-1.5">Email</label>
              <input name="email" type="email" placeholder="rahul@example.com" value={formData.email} onChange={handleChange} required
                className="w-full bg-zinc-900/50 border border-zinc-800 text-white placeholder-zinc-600 rounded-xl px-4 py-3 text-sm focus:border-zinc-600 focus:outline-none transition-colors" />
            </div>
            <div>
            ...
            <div>
              ...
              <div className="relative">
                <input name="password" type={showPassword ? 'text' : 'password'} placeholder="At least 8 characters" value={formData.password} onChange={handleChange} required
                  className="w-full bg-zinc-900/50 border border-zinc-800 text-white placeholder-zinc-600 rounded-xl px-4 py-3 pr-11 text-sm focus:border-zinc-600 focus:outline-none transition-colors" />
            ...
            <div>
              ...
              <div className="relative">
                <input name="confirmPassword" type={showConfirm ? 'text' : 'password'} placeholder="Re-enter your password" value={formData.confirmPassword} onChange={handleChange} required
                  className="w-full bg-zinc-900/50 border border-zinc-800 text-white placeholder-zinc-600 rounded-xl px-4 py-3 pr-11 text-sm focus:border-zinc-600 focus:outline-none transition-colors" />
        ...
        <p className="mt-4 text-center text-xs text-zinc-700">
          ...
          <Link href="/terms" className="text-zinc-500 hover:text-zinc-300">Terms</Link> and{' '}
