...
export default function InfluencerDealsPage() {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      if (!user) return;
      const { data } = await supabase.from('deals').select('*').eq('influencer_id', user.id).order('created_at', { ascending: false });
      setDeals(data || []);
      setIsLoading(false);
    })();
  }, []);
    ...
    <div className="space-y-6">
      <motion.div variants={fade} initial="hidden" animate="visible" custom={0}>
        ...
        <p className="text-zinc-500 text-sm mt-0.5">
          {deals.length === 0 ? 'No deals yet' : `${deals.length} deal${deals.length !== 1 ? 's' : ''} · ₹${stats.total.toLocaleString('en-IN')} total value`}
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Completed', value: stats.completed, icon: CheckCircle, color: 'text-blue-400', bg: 'bg-blue-500/10' },
          { label: 'Paid', value: stats.paid, icon: IndianRupee, color: 'text-green-400', bg: 'bg-green-500/10' },
          { label: 'Total Value', value: `₹${stats.total.toLocaleString('en-IN')}`, icon: Handshake, color: 'text-white', bg: 'bg-zinc-800' },
        ].map((s, i) => (
      ...
      {filteredDeals.length === 0 ? (
        <motion.div variants={fade} initial="hidden" animate="visible" custom={6}
          ...
          <p className="text-zinc-500 text-sm">{filter === 'all' ? 'Apply to campaigns to get your first deal' : 'Nothing here at the moment'}</p>
        </motion.div>
      ) : (
        <div className="space-y-3">
          {filteredDeals.map((deal, i) => (
            <motion.div key={deal.id} variants={fade} initial="hidden" animate="visible" custom={i + 6}
              onClick={() => router.push(`/influencer/deals/${deal.id}`)}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <div className="text-right">
                    ...
                    {deal.status === 'paid' && <p className="text-xs text-green-400">You received ₹{(deal.amount * 0.9).toLocaleString('en-IN')}</p>}
                  </div>
